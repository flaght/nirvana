...
class Volume(object):
    ...
    def to_csv(self):
        dict = {'symbol': self.__symbol, 'trader_id': self.__trader_id,
                'order_id': self.__order_id, 'direction': self.__direction.value,
                'price': round(self.__limit_price, 2), 'comb_offset': self.__comb_offset_flag.value,
                'cost': round(self.cost(), 2), 'fee': round(self.fee(), 2),
                'commission': round(self.__commission, 2), 'stamp':round(self.__stamp, 2),
                'transfer': round(self.__transfer, 2), 'daily_profit': round(self.__daily_profit, 2),
                'settle_price': round(self.__daily_settle_price, 2),
                'amount': self.__amount, 'min_volume': self.__min_volume,
